...
app_public_url = 'https://c09e-197-146-208-172.ngrok-free.app'
app_public_gather_url = app_public_url + '/gather'
app_public_event_url = app_public_url + '/event'

# Twilio credentials and OpenAI API Key
account_sid = 'account _ user id '
auth_token = 'auth token key'
openai_api_key = 'sk-*****'
gclient = Groq(api_key='groq api key')
openai.api_key = 'sk-*****'
ELEVENLABS_API_KEY = 'api token elevenlabs'
VOICE_ID = 'jsCqWAovK2LkecY7zXl4'

# Twilio number and customer number
from_number = '+14388069138'
...
def gather_input():
    """Gathers customer's speech input."""
    resp = VoiceResponse()
    gather = Gather(input='speech', action='/process-speech', speechTimeout='auto')
    resp.append(gather)
    # If customer doesn't respond, loop back
